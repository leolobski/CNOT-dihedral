...
The standard circuit model for quantum computation is based on the universal set of unitaries $CNOT, X, T$ and $H$, which generate the Clifford+T gate set. Universality here means that we can get arbitrarily close to any $n$-qubit unitary using polynomially many gates from this set.

The main focus for this blog post is on how to develop and exploit an algebraic theory of quantum circuits.
...
where $|± \rangle = \frac{1}{\sqrt 2}(|0\rangle ±  |1\rangle)$ and $\alpha\in(-\pi,\pi]$ (see e.g. [Backens](https://arxiv.org/abs/1307.7025) for a proper definition). The spiders can be thought of as generalised rotations; indeed, for a single qubit (n=1) they correspond to rotating the second basis vector by degree $\alpha$.
...
## Diagonal gates in ZX: reducing $T$-count in ZX circuits.

We have seen that phase polynomials are a useful technique for normalising diagonal circuits and distinguishing between them. Degree reduction rules and rule 13 in the paper are directly imported from this formalism. We have seen that most of those rules can be derived straightforwardly from the ZX calculus, except for rule 13, for which no direct derivation is known. In this section we analyse this rule from a ZX perspective, and show how it can be used to reduce T-count in ZX circuits.
